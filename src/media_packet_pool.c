...
{
	int ret = MEDIA_PACKET_POOL_ERROR_NONE;
	...
	if (pool_handle != NULL)
		...
		LOGE("out of memory");
		ret = MEDIA_PACKET_POOL_ERROR_OUT_OF_MEMORY;
		return ret;
	}
...
int media_packet_pool_set_media_format(media_packet_pool_h pool, media_format_h fmt)
{
	...
	if (MEDIA_FORMAT_GET_REFCOUNT(fmt) < 1) {
		...
		return MEDIA_PACKET_POOL_ERROR_INVALID_PARAMETER;
	}
	...
	if (!MEDIA_FORMAT_IS_VIDEO(fmt) && !MEDIA_FORMAT_IS_AUDIO(fmt)) {
		...
		return MEDIA_PACKET_POOL_ERROR_INVALID_OPERATION;
	}
...
int _allocate_pkt(media_packet_pool_h pool)
{
	...
	MEDIA_PACKET_POOL_NULL_ARG_CHECK(fmt_handle);
...
int media_packet_pool_allocate(media_packet_pool_h pool)
{
	...
	for (i = pool_handle->curr_pool_size; i < pool_handle->min_pool_size; i++) {
		if (_allocate_pkt(pool_handle) != MEDIA_PACKET_POOL_ERROR_NONE) {
...
int media_packet_pool_acquire_packet(media_packet_pool_h pool, media_packet_h * pkt)
{
	...
	ClockTime timeout;
	gint64 wait_until = -1;
	timeout = MEDIA_PACKET_POOL_AQUIRE_TIMEOUT;
	MEDIA_PACKET_POOL_INSTANCE_CHECK(pool);
	pool_handle = (media_packet_pool_s *) pool;

	if (timeout != CLOCK_TIME_NONE) {
		gint64 add = timeout / (SECOND / G_TIME_SPAN_SECOND);
	...
	if (!g_queue_is_empty(pool_handle->queue)) {
		pkt_handle = g_queue_pop_head(pool_handle->queue);
	} else if (timeout == CLOCK_TIME_NONE) {

		if (pool_handle->curr_pool_size < pool_handle->max_pool_size) {
			LOGI("Queue is less that max size, allocating one more packet");
		...
		LOGI("Queue is Empty, waiting for till release is done ");
		...
		LOGE("Queue is Empty, waiting for given timeout ");
