 ...
 * @details	Reuses the media packet by adding it into the media packet pool.
 *
 * @since_tizen 3.0
 *
 * @param[in]  pool The media packet pool handle
 * @param[in]  pkt The pkt should have previously been allocated from the pool with media_packet_pool_acquire_packet()
 *
 * @return @c 0 on success,
 *         otherwise a negative error value
 * @retval #MEDIA_PACKET_POOL_ERROR_NONE Successful
